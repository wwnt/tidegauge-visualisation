...
//var commonUrl = 'https://oceans.navi-tech.net/api/'//this is the generic api url
 ...
 const basePath = window.location.pathname.split('/')[1]; // 获取一级路径（如 'frontend'）


  // 构建 API 基础地址 Build the API base address
  const commonUrl = `${protocol}//${hostname}${port ? `:${port}` : ''}${basePath ? `/${basePath}` : ''}/api/`;
  // 根据当前协议动态选择 WebSocket 协议 Dynamically select the WebSocket protocol based on the current protocol
  const wsProtocol = protocol === 'https:' ? 'wss:' : 'ws:';
  const wsBaseUrl = `${wsProtocol}//${hostname}${port ? `:${port}` : ''}${basePath ? `/${basePath}` : ''}/api/ws/`;
...
const config = {
  ...
  //seaHeightPhoto: `${protocol}//${hostname}${port ? `:${port}` : ''}/${basePath}/sea_height_photo/`,//海平面图片所在的文件夹 The folder where the sea level images are located
  //reportUrl: `${protocol}//${hostname}${port ? `:${port}` : ''}/${basePath}/report/`,//报告路径 report url
