<template>
  <v-dialog v-model="display" persistent :width="dialogWidth">
    <template v-slot:activator="{ on }">
      <v-text-field v-bind="textFieldProps" :rules="[v => !!v || errorMsg]" :disabled="disabled" :loading="loading"
    ...
    <v-card>
      <v-card-text class="px-0 py-0">
        <v-tabs fixed-tabs v-model="activeTab">
          <v-tab key="calendar">
            <slot name="dateIcon">
              <v-icon>{{$vuetify.lang.t('$vuetify.datetimePicker.date')}}</v-icon>
            </slot>
          </v-tab>
          <v-tab key="timer" :disabled="dateSelected">
            <slot name="timeIcon">
              <v-icon>{{$vuetify.lang.t('$vuetify.datetimePicker.time')}}</v-icon>
            </slot>
          </v-tab>
          ...
          <v-tab-item key="timer">
            <v-time-picker ref="timer" class="v-time-picker-custom" v-model="time" format="24hr"
              v-bind="timePickerProps" full-width></v-time-picker>
          </v-tab-item>
        </v-tabs>
      </v-card-text>
      <v-card-actions>
        ...
        <slot name="actions" :parent="this">
          <v-btn color="grey lighten-1" text @click.native="clearHandler">{{
            $vuetify.lang.t('$vuetify.datetimePicker.cancel')
          }}</v-btn>
          <v-btn color="green darken-1" text @click="okHandler">{{
            $vuetify.lang.t('$vuetify.datetimePicker.confirm')
          }}</v-btn>
        </slot>
      </v-card-actions>
    </v-card>
